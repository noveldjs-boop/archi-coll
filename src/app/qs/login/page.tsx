...
import { Lock, Mail, Calculator, ArrowLeft } from 'lucide-react'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Check if user is a QS
        const response = await fetch('/api/members/me')
        if (response.ok) {
          const userData = await response.json()
          if (userData.member?.profession === 'qs') {
            router.push('/qs/dashboard')
          } else {
            setError(`Anda login sebagai ${userData.member?.profession || 'member'}, bukan Quantity Surveyor`)
          }
        } else {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
          <CardHeader className="space-y-2 text-center">
            ...
            <CardTitle className="text-2xl text-white">Quantity Surveyor Login</CardTitle>
            <CardDescription className="text-gray-400">
              Masuk ke dashboard Quantity Surveyor ARCHI-COLL
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-[#9B59B6] to-[#8E44AD] hover:from-[#8E44AD] hover:to-[#7D3C98] text-white"
                disabled={loading}
              >
