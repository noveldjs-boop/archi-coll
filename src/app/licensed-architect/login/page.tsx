...
import { Lock, Mail, Award, ArrowLeft } from 'lucide-react'
import { toast } from '@/hooks/use-toast'

export default function LicensedArchitectLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Check if user is a licensed architect
        const response = await fetch('/api/members/me')
        if (response.ok) {
          const userData = await response.json()
          if (userData.member?.profession === 'licensed-architect') {
            router.push('/licensed-architect/dashboard')
          } else {
            setError(`Anda login sebagai ${userData.member?.profession || 'member'}, bukan Licensed Architect`)
          }
        } else {
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
          <CardHeader className="space-y-2 text-center">
            <div className="flex justify-center mb-4">
              <div className="bg-gradient-to-br from-[#C0392B] to-[#E74C3C] p-4 rounded-full">
                <Award className="w-8 h-8 text-white" />
              </div>
            </div>
            <CardTitle className="text-2xl text-white">Licensed Architect Login</CardTitle>
            <CardDescription className="text-gray-400">
              Masuk ke dashboard Licensed Architect ARCHI-COLL
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-[#C0392B] to-[#E74C3C] hover:from-[#A93226] hover:to-[#CB4335] text-white"
                disabled={loading}
              >
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-400">
                Akun Demo: licensed@demo.com / licensed123
              </p>
            </div>
          </CardContent>
        </Card>
