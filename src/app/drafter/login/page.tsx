...
import { Lock, Mail, PencilRuler, ArrowLeft } from 'lucide-react'
import { toast } from '@/hooks/use-toast'

export default function DrafterLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        if (response.ok) {
          const userData = await response.json()
          if (userData.member?.profession === 'drafter') {
            router.push('/drafter/dashboard')
          } else {
            setError(`Anda login sebagai ${userData.member?.profession || 'member'}, bukan Drafter`)
          }
        } else {
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
          <CardHeader className="space-y-2 text-center">
            <div className="flex justify-center mb-4">
              <div className="bg-gradient-to-br from-[#16A085] to-[#1ABC9C] p-4 rounded-full">
                <PencilRuler className="w-8 h-8 text-white" />
              </div>
            </div>
            <CardTitle className="text-2xl text-white">Drafter Login</CardTitle>
            <CardDescription className="text-gray-400">
              Masuk ke dashboard Drafter ARCHI-COLL
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-[#16A085] to-[#1ABC9C] hover:from-[#138D75] hover:to-[#17A589] text-white"
                disabled={loading}
              >
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-400">
                Akun Demo: drafter@demo.com / drafter123
              </p>
            </div>
          </CardContent>
        </Card>
