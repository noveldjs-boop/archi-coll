...
export default function MEPLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Check if user is a MEP engineer
        const response = await fetch('/api/members/me')
        if (response.ok) {
          const userData = await response.json()
          if (userData.member?.profession === 'mep') {
            router.push('/mep/dashboard')
          } else {
            setError(`Anda login sebagai ${userData.member?.profession || 'member'}, bukan MEP Engineer`)
          }
        } else {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
          <CardHeader className="space-y-2 text-center">
            ...
            <CardTitle className="text-2xl text-white">MEP Engineer Login</CardTitle>
            <CardDescription className="text-gray-400">
              Masuk ke dashboard MEP Engineer ARCHI-COLL
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-[#3498DB] to-[#2980B9] hover:from-[#2980B9] hover:to-[#1F618D] text-white"
                disabled={loading}
              >
